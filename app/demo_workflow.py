...
def demo_doctor_search():
    ...
    try:
        ...
        print(f"Search for '{query}' found {len(doctors)} doctors")

        for i, doctor in enumerate(doctors[:3]):  # Show first 3 results
            print(f"  {i+1}. {doctor.get('姓名', 'Unknown')} - {doctor.get('擅长', 'Unknown')}")
...
def demo_conversation_management():
    ...
    # Create conversation
    success = conversation_service.create_conversation(conversation_id)
    print(f"Created conversation: {success}")

    # Add messages
    messages = [
        ConversationMessage(role="user", content="你好"),
        ConversationMessage(role="assistant", content="您好！我是小惠，很高兴为您服务。"),
        ConversationMessage(role="user", content="我想咨询感冒治疗")
    ]

    for msg in messages:
        success = conversation_service.add_message(conversation_id, msg, "demo")
